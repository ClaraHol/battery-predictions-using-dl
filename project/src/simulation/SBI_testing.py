...
def run_multiple_checks(prior, n_checks=30, max_attempts=400):
    ...
    while len(all_errors) < n_checks and attempts < max_attempts:
        attempts += 1
        true_features, y_obs, was_extreme = generate_voltage_curve(
            prior
        )  # have it return a flag
        if was_extreme:
            n_extreme_skipped += 1
            continue

        posterior_samples = posterior.sample(
            (50000,), x=y_obs, show_progress_bars=False
        )
        posterior_means = posterior_samples.mean(dim=0)
        normalized_error = (
            (posterior_means - true_features.squeeze(0)) / prior_range
        ) ** 2
