...
def safe_process(df, cell_id, source_dataset, errors):
    ...
    try:
        return process_cell(
            df, cell_id, source_dataset, efc_early=EFC_EARLY, efc_late=EFC_LATE
        )
...
def main():
    ...
    if elt_files:
        print(
            "  [SKIPPED] parse_lgmj1() needs temp_C_nameplate per file, decoded "
    ...
    if snl_files:
        print(
            "  Attempting parse with UNVERIFIED schema - failures here are "
    ...
    if not metadata_rows:
        ...
        if efc_early_frames:
            ...
            efc_early_all.to_parquet(
                OUTPUT_DIR / "cycle_early_curves.parquet", index=False
            )
            print(
                f"cycle EFC_EARLY curves for surviving cells -> {OUTPUT_DIR / 'cycle_early_curves.parquet'}"
            )
        else:
            print(
                f"No cell had data at cycle_number == {EFC_EARLY} - check EFC_EARLY "
        ...
        if efc_late_frames:
            ...
            efc_late_all.to_parquet(
                OUTPUT_DIR / "cycle_late_curves.parquet", index=False
            )
            print(
                f"cycle EFC_LATE curves for surviving cells -> {OUTPUT_DIR / 'cycle_late_curves.parquet'}"
            )
