...
import pandas as pd
from parsers import (
    build_pouch_cells_cell_record,
    extract_primary_charging_curve,
    parse_pouch_cells_rpt_curve,
    parse_pouch_cells_rpt_metadata,
)

DATA_ROOT = Path("/work3/claho/battery_datasets")
OUTPUT_DIR = Path("/work3/claho/battery_datasets/processed/pouch_cells")
...
def find_cell_pairs(raw_dir: Path):
    ...
    complete = {
        cid: files
        for cid, files in pairs.items()
        if "metadata" in files and "parquet" in files
    }
...
def main(raw_dir: str):
    ...
    if incomplete:
        print(
            f"Skipping {len(incomplete)} cell(s) missing one of the two required files: "
    ...
    for cell_id, files in complete.items():
        ...
        print(
            f"  nominal_capacity_Ah={record['nominal_capacity_Ah']:.2f}, "
            f"cycle_life_rpt_units={record['cycle_life_rpt_units']}, "
            f"cycle_life_efc_units={record['cycle_life_efc_units']:.1f}"
        )
        print(
            f"  EFC{TARGET_EFC} brackets: RPT{record['efc50_rpt_before']} <-> "
        ...
        curve_before = parse_pouch_cells_rpt_curve(
            files["parquet"], rpt_number=rpt_before
        )
        efc50_before = extract_primary_charging_curve(
            curve_before
        )  # Extract the charging curve
        ...
        if rpt_after != rpt_before:
            curve_after = parse_pouch_cells_rpt_curve(
                files["parquet"], rpt_number=rpt_after
            )
            efc50_after = extract_primary_charging_curve(
                curve_after
            )  # Extract the charging curve
    ...
    if rpt0_frames:
        pd.concat(rpt0_frames, ignore_index=True).to_parquet(
            OUTPUT_DIR / "pouch_cells_rpt0_curves.parquet", index=False
        )
    ...
    if efc50_before_frames:
        ...
        print(
            f"EFC{TARGET_EFC}-before curves -> {OUTPUT_DIR / 'pouch_cells_efc50_before_curves.parquet'}"
        )
    if efc50_after_frames:
        ...
        print(
            f"EFC{TARGET_EFC}-after curves -> {OUTPUT_DIR / 'pouch_cells_efc50_after_curves.parquet'}"
        )
