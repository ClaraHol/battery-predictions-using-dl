...
# Specific parameters
capa_used = 2.6576  # Discharge or charge capacity at the target cycle number (i.e. cycle 1) [Ah], take the minimal value of all similar cells.
# Nominal capacity
...
# area = 952e-4
area = 957e-4  # cite: Thickness changeandjellyroll deformationanditsimpactontheagingand
# lifetime of commercial 18650cylindrical Li-ion cells with silicon containing
# anodesandnickel-rich cathodes
# Thichness [m]
# L_n = 53e-6
L_n = 60.6e-6  # cite Benz
L_p = 53e-6  # cite Benz
L_s = 7e-6  # Average taken from 2 papers
# Porosity [-]
epse_n = 0.38
epse_p = 0.15
epse_s = 0.393  # This is (0.38+0.38+0.42)/3
# Particle radius [m]
Rp_n = 10e-6  # half particle size
Rp_p = 5e-6  # half particle size
######################################################################
...
def anode_ocp(sto):  # SiOx CMU anode, 0 - 364.11/448 , discharge
    cmu_1 = 364.11 / 448
    sto = sto / cmu_1
    a1 = -2.06052828e00
    a2 = 6.27935878e00
    ...
    h6 = -5.90347378e-01
    u_eq = (
        a1 * pybamm.exp(a2 * sto)
        + a3
        + a4 * pybamm.exp(a5 * sto)
        + a6 * (sto - 0.55) ** 2
        + a7 * pybamm.exp(a8 * sto)
...
def cathode_ocp(sto):  # NCA CMU cathode, 0 - 167.414/306 , discharge
    sto = 1 - sto
    cmu_1 = 167.414 / 306
    sto = sto / cmu_1
    a1 = -2.10087414e00
...
def params_setting(params):
    ...
    params_dict = {
        ...
        "Initial concentration in negative electrode [mol.m-3]": Cs_max_n
        * theta_n_init,
        "Initial concentration in positive electrode [mol.m-3]": Cs_max_p
        * theta_p_init,
