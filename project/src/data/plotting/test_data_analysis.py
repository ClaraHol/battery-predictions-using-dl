...
# Set up
DATA_ROOT = Path("/work3/claho/battery_datasets/pouch_cells/test_data")
BATTERY_TYPE = "PA-b1"
...
for file in files:
    ...
    # Plot voltage
    ax.plot(rpt1["time_step"], rpt1["voltage (V)"], linewidth=1, label=cell_id)
    ...
    ax.set_ylim(rpt1["voltage (V)"].min() - 0.05, rpt1["voltage (V)"].max() + 0.15)
